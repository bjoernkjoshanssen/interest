...
theorem exercise_1_2_chan_tse_pos {u x : ℝ} (hu : 0 < u) (hx : 1 < x) :
  ...
  have h₀ : 1 + u / x ≠ 0 := by
    contrapose! h₃
    linarith
  have h₂ : u / (x + u) ≠ 0 := ne_of_gt <| div_pos hu (by linarith)
...
theorem exercise_1_2_chan_tse_deriv₀ {u x : ℝ} (hu : 0 < u) (hx : 1 < x) :
  ...
  rw [deriv_div]
  simp
  exact exercise_1_2_chan_tse_pos hu hx
...
theorem exercise_1_2_chan_tse_deriv {u : ℝ} (hu : 0 < u) :
  ...
  have : deriv (fun t ↦ (1 + u / t) ^ t) x
       ...
       Filter.EventuallyEq.deriv_eq <| eventually_eventuallyEq_nhds.mp <|
        ...
        simp at hy ⊢
        rw [mul_comm]
        apply rpow_def_of_pos <| add_pos (by simp) <| div_pos hu (by linarith)
  rw [this]
  exact exercise_1_2_chan_tse_deriv₀ hu hx
...
theorem rational_exponent_interest_le_integer {ε m n k : ℝ} (hε : 0 < ε) (hk : 1 < k)
  ...
  · apply (continuousOn_congr (by
      show Set.EqOn (fun t ↦ rexp (t * log (1 + u / t))) (fun t ↦ (1 + u / t) ^ t) (Set.Ici 1)
      intro t ht
      simp at ht
      have : 0 < 1 + u / t := by
        apply add_pos;simp;apply div_pos;tauto
        linarith
      simp
      rw [mul_comm]
    ...
    exact ContinuousOn.rexp <| by
      apply ContinuousOn.mul (continuousOn_id' (Set.Ici 1))
      ...
      · intro x hx
        simp at hx
        apply ne_of_gt <| add_pos (by simp) <| div_pos hu <| by linarith

  exact exercise_1_2_chan_tse_deriv hu
  · simp
  · simp
    linarith
  · exact hk
...
not by definition.
-/
lemma chan_tse_exercise_1_6_b {a A i I : ℝ → ℝ} (c : ℝ)
    (h1_6 : ∀ t, a t = log (t^2 / 2 + exp 1) + t^((3:ℝ) / 10) / 20)
...
`nomDis` is good. -/
example (d m : ℝ) (hd : 0 ≤ 1 - d) (hm : m ≠ 0): (1 - (d.nomDis m) / m) ^ (m:ℝ) = 1 - d := by
  simp [Real.nomDis]
  have hmm : m⁻¹ * m = 1 := by
    refine inv_mul_cancel₀ hm
  have hq : ((1 - m * (1 - (1 - d) ^ m⁻¹) / m) ^ m) ^ m⁻¹
    = ((1 - m * (1 - (1 - d) ^ m⁻¹) / m) ^ (m * m⁻¹))  := by
      rw [rpow_mul]
      suffices m * (1 - (1 - d) ^ m⁻¹) / m ≤ 1 by linarith
      rw [mul_comm]
      have : (1 - (1 - d) ^ m⁻¹) * m / m =
              (1 - (1 - d) ^ m⁻¹) := by
        generalize (1 - (1 - d) ^ m⁻¹)  = α
        field_simp
      rw [this]
      suffices 0 ≤ (1 - d) ^ m⁻¹ by linarith
      apply rpow_nonneg
      exact hd


  suffices  ((1 - m * (1 - (1 - d) ^ m⁻¹) / m) ^ m) ^ ((1:ℝ)/ m) = (1 - d)^ ((1:ℝ)/ m) by
    have := (@rpow_left_inj (z := 1 / m) (x := ((1 - m * (1 - (1 - d) ^ m⁻¹) / m) ^ m))
      (y := 1 - d) (by
        apply rpow_nonneg
        suffices m * (1 - (1 - d) ^ m⁻¹) / m ≤ 1 by
          linarith
...
example (i : ℝ) : i.nomInt 1 = i := by
  unfold nomInt
  simp

example (i x : ℝ) (n : ℝ) (hi : 0 ≤ i) (hn : n > 0) (h : 1 + x = (1 + i / n) ^ (n:ℝ)) :
  i = n * ((1 + x) ^ (1 / n) - 1) := by
...
example (i : ℝ) : i.nomInt 2 < i := by
  simp [nomInt]
  suffices ((1 + i) ^ (2:ℝ)⁻¹) < 1 + i / 2 by linarith
  suffices ((1 + i) ^ (2:ℝ)⁻¹) ^ 2 < (1 + i / 2) ^ 2 by sorry
  sorry
