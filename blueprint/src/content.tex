    ...
    Using the Lean proof assistant \cite{demoura21lean4}, we verify the unique solvability of the Annuity Equation for each variable under a non-par bond assumption.
...
Main result: if $r > i$ and $d < 1 + 1 / i$ then we can recover $n$ (maturity) from $D$ (Macaulay duration \cite{Macaulay1938Problems}) and $r$ and $i$.
...
$d > 1 + 1/i$ &	cannot recover (\emph({AriExtremum.lean}, Chan and Tse figure))		& impossible\\
\end{tabular}
